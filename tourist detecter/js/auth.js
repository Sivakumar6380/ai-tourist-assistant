...
const Auth = {
        ...
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
            navigator.mediaDevices.getUserMedia({ video: { width: 320, height: 240 } })
                .then(stream => {
                    ...
                    this.runScanSequence(stream, statusEl, progressEl);
                })
                .catch(err => {
                    statusEl.textContent = "SIMULATING RETINAL BIOMETRICS...";
                    this.runScanSequence(null, statusEl, progressEl);
                });
        } else {
            statusEl.textContent = "SIMULATING RETINAL BIOMETRICS...";
            this.runScanSequence(null, statusEl, progressEl);
    ...
    runScanSequence(stream, statusEl, progressEl) {
        let pct = 0;
        const steps = [
        ...
        const interval = setInterval(() => {
            ...
            if (pct >= 100) {
                ...
                // Default to Rahul Sharma (T001) for demo
                const rahul = DB.getTourist('T001');
                setTimeout(() => {
                    const overlayEl = document.getElementById('face-scanner-overlay');
                    if (overlayEl) overlayEl.remove();
                    
                    if (rahul) {
                        Utils.toast('Face Match Found', 'Welcome back, Rahul Sharma!', 'success');
                        this.login(rahul);
                    } else {
                        Utils.toast('Verification Failed', 'No blockchain identity registered.', 'danger');
                    }
                }, 800);
            }
        }, 120);
